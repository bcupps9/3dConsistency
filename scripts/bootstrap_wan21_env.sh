...
# Bootstraps a dedicated Wan2.1 environment on cluster/local, with optional
# diagnostics for flash-attn runtime/linker issues (GLIBC / .so resolution).
#
# Usage:
...
#   PURGE_PIP_CACHE=1
#   VERBOSE_PIP=0
#   DIAG_DIR=~/projects/3dConsistency/.diag/wan21_manual
...
DIAG_DIR="${DIAG_DIR:-$PROJECT_DIR/.diag/wan21_$(date +%Y%m%d_%H%M%S)}"
...
say "Install Wan2.1 Requirements (Without flash-attn)"
...
if [[ "${INSTALL_FLASH_ATTN}" == "1" ]]; then
  ...
  if ! command -v nvcc >/dev/null 2>&1; then
    ...
    export MAX_JOBS
    export TORCH_EXTENSIONS_DIR="/tmp/torch_extensions_${USER}_wan21"
